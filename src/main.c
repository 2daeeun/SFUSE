...
int main(int argc, char *argv[]) {
  ...
  /*
   * dev_path: 마운트할 블록 디바이스의 경로를 저장하는 포인터
   * mountpoint: SFUSE 파일시스템을 마운트할 디렉토리 경로를 저장하는 포인터
   */
   ...
   *
   * open(): 파일이나 디바이스를 열 때 사용.
   * O_RDWR: 읽기 및 쓰기 모드,
   * O_SYNC: 데이터를 즉시 디스크에 쓰는 동기화 모드를 설정한다.
   */
   ...
   *
   * fstat(): 파일 디스크립터의 상태를 stat 구조체에 저장하며,
  ...
  /*
   * SFUSE 파일시스템 관리를 위한 구조체(sfuse_fs)에 메모리를 할당하고 초기화.
  ...
  /* 명령줄에서 추가로 입력된 모든 옵션을 FUSE 인자에 추가 */
   ...
   * -o:
   *  FUSE에 옵션을 전달할 때 사용하는 플래그이다.
   *  (여러 옵션을 쉼표로 구분하여 함께 전달할 수 있다.)
   *
   * - allow_other:
   *   다른 사용자가 마운트된 파일시스템에 접근할 수 있도록 허용한다.
   *
   * - default_permissions:
   *   파일시스템의 기본적인 권한 관리를 FUSE가 수행하도록 한다.
   *
   * -d:
   *  파일시스템의 동작과 관련된 자세한 디버깅 정보를 출력한다.
   */
